...
public static class IonizationDataProvider
{
    ...
    private static IonizationChannelInfo Describe(IonizationChannelSpec spec, double e0KeV, IonizationFsTable table)
    {
        ...
        var partial = new IonizationChannelInfo
        {
            ...
            //260809Cl 変更: 生成器の正本が Temari (github.com/seto77/Temari) に移ったので出所をそう名乗る
            //(旧: "self-generated DHFS tables (tools/IonizationGen prod)")。ReciPro はテーブルの消費者で、
            //処方と精度を語る正本は Temari 側にある — 二か所で説明すると必ずずれるため (作者判断 2026-08-09)。
            ShapeSource = new IonizationDataProvenance(table.ModelId, table.DatasetVersion,
                "generated by Temari (github.com/seto77/Temari) — first-principles isolated-atom DHFS/Dirac tables")
        };
