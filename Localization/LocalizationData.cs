...
// 260621Cl 自動生成 (tools/i18n_s25_gen_data.py 由来)。
// 260726Cl 追記: 生成スクリプトの入力 (C:/tmp/*.json 等) は当時のセッション産物で再現できないため、以降は
//   このファイルが実質の正本。文字溢れ対策で一部の訳語を短縮する等の手編集を入れている (各行の // 260726Cl 参照)。
...
internal static class LocalizationData
{
    public static void Populate(System.Collections.Generic.Dictionary<string, Localization.Entry[]> reg)
    {
        ...
        reg["FormDiffractionSimulatorDynamicCompression"] = new Localization.Entry[]
        {
            ...
            new("groupBoxCompressedRotation", "Text", "Rotation distribution", "回転分布", "Rotationsverteilung", "Distrib. de rotation", "Distrib. de rotación", "Distrib. de rotação", "Distrib. di rotazione", "Распр. поворотов", "旋转分布", "旋轉分布", "회전 분포"),
            new("groupBoxReleasedRotation", "Text", "Rotation distribution", "回転分布", "Rotationsverteilung", "Distrib. de rotation", "Distrib. de rotación", "Distrib. de rotação", "Distrib. di rotazione", "Распр. поворотов", "旋转分布", "旋轉分布", "회전 분포"),
            ...
            new("checkBoxSkipDrawing", "Text", "Skip drawing during execution", "実行中は描画を省略", "Zeichnen überspringen", "Ignorer le tracé", "Omitir el dibujo", "Pular o desenho", "Salta il disegno", "Без отрисовки", "执行期间跳过绘制", "執行期間略過繪製", "실행 중 그리기 건너뛰기"),
            ...
            new("radioButtonCompressedUniaxial", "Text", "Uniaxial compression", "一軸圧縮", "Einachsige Kompression", "Compression uniaxiale", "Compresión uniaxial", "Compressão uniaxial", "Compr. uniassiale", "Одноосное сжатие", "单轴压缩", "單軸壓縮", "단축 압축"),
            new("groupBoxReleasedArea", "Text", "Released area", "解放領域", "Freigegebener Bereich", "Zone libérée", "Área liberada", "Área liberada", "Area rilasciata", "Освоб. область", "释放区域", "釋放區域", "해방 영역"),
            ...
            new("radioButtonReleasedUniaxial", "Text", "Uniaxial compression", "一軸圧縮", "Einachsige Kompression", "Compression uniaxiale", "Compresión uniaxial", "Compressão uniaxial", "Compr. uniassiale", "Одноосное сжатие", "单轴压缩", "單軸壓縮", "단축 압축"),
            new("groupBoxSampleParameters", "Text", "Sample parameters", "試料パラメータ", "Probenparameter", "Param. échantillon", "Parámetros de la muestra", "Parâmetros da amostra", "Parametri del campione", "Параметры образца", "样品参数", "試樣參數", "시료 매개변수"),
            new("label4", "Text", "Mass absorption\r\ncoefficient", "質量吸収\r\n係数", "Massen-\r\nabsorptionskoeffizient", "Coefficient d'absorption\r\nmassique", "Coeficiente de absorción\r\nmásica", "Coeficiente de absorção\r\nde massa", "Coeff. di assorbimento\r\ndi massa", "Коэффициент массового\r\nпоглощения", "质量吸收\r\n系数", "質量吸收\r\n係數", "질량 흡수\r\n계수"),
        ...
        reg["FormDiffractionSpotInfo"] = new Localization.Entry[]
        {
            ...
            new("label16", "Text", "Note 1: Unit of length is \"nm\", not \"Å\".   Note 2: Unit of wavenumber  is \"1/nm\", not \"2π/nm\".", "注1: 長さの単位は \"Å\" ではなく \"nm\" です。   注2: 波数の単位は \"2π/nm\" ではなく \"1/nm\" です。", "Anmerkung 1: Längeneinheit ist \"nm\", nicht \"Å\".   Anmerkung 2: Wellenzahl in \"1/nm\", nicht \"2π/nm\".", "Note 1 : longueur en « nm », pas « Å ».   Note 2 : nombre d'onde en « 1/nm », pas « 2π/nm ».", "Nota 1: longitud en \"nm\", no \"Å\".   Nota 2: número de onda en \"1/nm\", no \"2π/nm\".", "Nota 1: comprimento em \"nm\", não \"Å\".   Nota 2: número de onda em \"1/nm\", não \"2π/nm\".", "Nota 1: lunghezza in \"nm\", non \"Å\".   Nota 2: numero d'onda in \"1/nm\", non \"2π/nm\".", "Прим. 1: длина в \"nm\", не \"Å\".   Прим. 2: волновое число в \"1/nm\", не \"2π/nm\".", "注 1: 长度单位为 \"nm\"，而非 \"Å\"。   注 2: 波数单位为 \"1/nm\"，而非 \"2π/nm\"。", "註 1：長度單位為「nm」，非「Å」。   註 2：波數單位為「1/nm」，非「2π/nm」。", "참고 1: 길이 단위는 \"Å\"가 아니라 \"nm\"입니다.   참고 2: 파수 단위는 \"2π/nm\"가 아니라 \"1/nm\"입니다."),
        ...
        reg["FormPolycrystallineDiffractionSimulator"] = new Localization.Entry[]
        {
            ...
            new("buttonLoadSetting", "Text", "Load setting", "設定を開く", "Laden", "Charger", "Cargar", "Carregar", "Carica", "Загрузить", "载入设置", "載入設定", "설정 불러오기"),
            new("buttonSaveCurrentSetting", "Text", "Save current setting", "現在の設定を保存", "Speichern", "Enregistrer", "Guardar", "Salvar", "Salva", "Сохранить", "保存当前设置", "儲存目前設定", "현재 설정 저장"),
            ...
            new("groupBoxPreferredOrientation", "Text", "Fitting parameters for preferred orientation", "選択配向のフィッティングパラメータ", "Fit-Parameter für Vorzugsorientierung", "Ajustement orientation préférentielle", "Parámetros de ajuste para orientación preferente", "Parâmetros de ajuste para orientação preferencial", "Parametri di fitting per l'orientazione preferenziale", "Подгонка преимущественной ориентации", "择优取向拟合参数", "擇優取向的擬合參數", "우선 배향 피팅 매개변수"),
            ...
            new("checkBoxRefineCenterOffset", "Text", "Center offset", "中心オフセット", "Zentrumsversatz", "Décal. du centre", "Desplaz. centro", "Desloc. centro", "Scost. centro", "Смещ. центра", "中心偏移", "中心偏移", "중심 오프셋"),
            new("checkBoxRefinePreferredOrientation", "Text", "Preferred \r\n orientation", "優先 \r\n 配向", "Vorzugs-\r\n orientierung", "Orientation \r\n préférentielle", "Orientación \r\n preferente", "Orientação \r\n preferencial", "Orientazione \r\n preferenziale", "Преимущ. \r\n ориентация", "择优 \r\n 取向", "擇優 \r\n 取向", "우선 \r\n 배향"),
            new("checkBoxRefineFilmBlur", "Text", "Film blur", "フィルムぼかし", "Filmunschärfe", "Flou du film", "Desenf. película", "Desf. do filme", "Sfoc. pellicola", "Размыт. плёнки", "胶片模糊", "底片模糊", "필름 블러"),
